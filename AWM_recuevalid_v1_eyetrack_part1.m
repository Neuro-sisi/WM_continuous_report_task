...
%% get subinfo
EEG_record = input('Have you clicked the EEG recording button?  yes/no   ', 's');
SubNo = input ('Please input subject number:  ');
PartNo = input ('Please input part number ((part1-1)):  ');
...
try
    for t = 1:TrialNum
        %     for t = 1:param.ntrials
        %% Break
        if  t == 1
            %             %% instruction-image
            %             % Get image and write to buffer
            %             Screen('FillRect', scr.wptr,[0 0 0]);
            %             img = Screen ('MakeTexture', scr.wptr, imread(sprintf ('InternalWMdis_taskinstruction_pv5.jpg')));
    ...
    if Eyetracking
        ...
        % Eyelink('Shutdown');
        movefile([pwd filesep edfname], ['recuevalid_v1_condi_' num2str(BlkSeq_cp(PartNo)) '_sub' edfname]); % move file to EDF
    end
    ...
    %% Save data
    save (fullfile (['recuevalid_v1_sub' num2str(SubNo)  '_part' num2str(PartNo) '_condi' num2str(BlkSeq_cp(PartNo))]));
    Screen('CloseAll');
